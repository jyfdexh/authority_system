...
//			Document document = Jsoup.parse(new File(PathUtils.getWebAppPath("static/iconfont/demo_index.html")), "utf-8");
			Document document = Jsoup.parse(new File("D:\\MyJava\\authority_system\\authority_web\\src\\main\\webapp\\static\\iconfont\\demo_index.html"), "utf-8");

			// 获取文档对象中所有的font-class类元素
			Element element = document.getElementsByClass("font-class").get(0);
			// 获取文档对象中所有的dib类元素
			Elements elements = element.getElementsByClass("dib"); // 获取所有图片li对象
			// 遍历处理，然后添加到list集合中
			for (Element element2 : elements) {  // .icon-accessory      icon-accessory
